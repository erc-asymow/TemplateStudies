{
  ...
  for(int ip = 0; ip<phasespace.size(); ip++ ){
    for(int ip2 = 0; ip2<proc.size(); ip2++ ){
          for(int idx = 0; idx<dx.size(); idx++ ){
	    for(int idy = 0; idy<dy.size(); idy++ ){
	      TFile* fin = TFile::Open("fout_fit_"+phasespace[ip]+"_"+proc[ip2]+"_UL_x"+dx[idx]+"_y"+dy[idy]+".root", "READ");
		...
		TH2D* d_i_clone = (TH2D*) d_i->Clone(phasespace[ip]+"_"+proc[ip2]+"_UL_x"+dx[idx]+"_y"+dy[idy]+"_"+TString(Form("%d",i)));
		...
		e_i->Draw("COLZ");
		c->SaveAs(TString(Form("scetlib_var%d", i))+"_"+phasespace[ip]+"_"+proc[ip2]+"_UL_x"+dx[idx]+"_y"+dy[idy]+".png");
		//return;
	      }
	      fin->Close();
	    }
