...
int main(int argc, char* argv[])
{
  ...
  if(runfit){
	...
	for(unsigned int idy=1; idy<=Y_nbins; idy++){
      ...
      TFile* fin_syst = TFile::Open(("/scratch/tanmay/Output2016/Final_Uses/V4/root_files_ul_"+xvar+"_scet_vars_2d.root").c_str(), "READ");	    
      if(fin_syst==0){
	...
	for(unsigned int idy=1; idy<=Y_nbins; idy++){
      ...
      TFile* fin_syst = TFile::Open(("/scratch/tanmay/Output2016/Final_Uses/V4/root_files_ang_coeff_"+xvar+"_pdf_msht20_vars_2d.root").c_str(), "READ");	    
      if(fin_syst==0){
	cout << "Cannot find syst file" << endl;
	return 0;
      }
	...
	TFile* fin_syst = TFile::Open( "/scratch/tanmay/Output2016/Final_Uses/V4/root_files_ang_coeff_"+TString(xvar.c_str())+"_pdf_"+iproc+"_vars_2d.root", "READ");
	if(fin_syst==0){
	  cout << "Cannot find syst file" << endl;
	  continue;
	}
  ...
  for(unsigned int i=0; i <proc.size(); i++){
    ...
    int nfpx = degf_map[iproc].at(0) + 1 - int(clip);
    int nfpy = degf_map[iproc].at(1)/2 + 1 - int(clip);
    //int nfp = nfpx*nfpy;
    ...
    hinfo->SetBinContent(11, xf_max);
    hinfo->GetXaxis()->SetBinLabel(11, "xmax");
    hinfo->SetBinContent(12, yf_max);
    hinfo->GetXaxis()->SetBinLabel(12, "ymax");
	...
	continue;
      for(unsigned int ipy = (!clip ? 0 : 1); ipy<(degf_map[iproc].at(1) + 1); ipy++){	  
	// it's a POI
