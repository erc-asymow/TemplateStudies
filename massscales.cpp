...
int main(int argc, char* argv[])
{
  ...
  TFile* fout = TFile::Open(("./massscale_"+tag+"_"+run+".root").c_str(), "RECREATE");
  ...
  dlast = std::make_unique<RNode>(dlast->Define("weight", [](float weight){
  ...
  const double x_low  = 75.0;
  const double x_high = 105.0;
  ...
  histos2D.emplace_back(dlast->Histo2D({"h_reco_bin_dm",  "nominal", n_bins, 0, double(n_bins), 20, -5.0, 5.0}, "index", "reco_dm", "weight"));
  //histos2D.emplace_back(dlast->Histo2D({"h_smear_bin_dm", "nominal", n_bins, 0, double(n_bins), 20, -5.0, 5.0}, "index", "smear_dm", "weight"));      
